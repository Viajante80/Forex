...
def evaluate_pair(i_pair, mashort, malong, price_data):
    ...
    return ma_result.MAResult(df_trades=df_trades, pairname=i_pair.name, params={"mashort": mashort, "malong": malong})


def get_price_data(pairname, granularity):
    df = pd.read_pickle(utils.get_data_filename(pairname, granularity))
